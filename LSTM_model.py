...
##################################################################
# کلاس HoursGene (اختیاری): تنظیم ماسک ساعت
##################################################################
class HoursGene:
    """
...
##################################################################
# 4) کلاس اصلی GeneticAlgorithmRunner
##################################################################
class GeneticAlgorithmRunner:
    ...
    def load_and_prepare_data(self, csv_file='XAUUSD_M30.csv'):
        """
    ...
    ###########################################################
    # 4.1) توابع موردنیاز DEAP (init_individual, evaluate_individual, ...)
    ###########################################################
    ...
    def evaluate_final(self, best_ind):
        ...
        num_features = X_train.shape[1] // window_size
        if (window_size * num_features) != X_train.shape[1]:
            print("Shape mismatch in final training.")
            return 0.0
        ...
        neg_th1, pos_th1, acc_th1, _, _ = tfinder.find_best_thresholds(y_proba_thresh, y_thresh_np)
        ...
        neg_th2, pos_th2, acc_test, _, _ = tfinder.find_best_thresholds(y_proba_test, y_test_np)
    ...
    ###########################################################
    # 4.2) متد اصلی main برای اجرای کل پروژه
    ###########################################################
    def main(self):
        """
        اجرای کل فرایند:
        ...
        for gen in range(1, self.N_GENERATIONS+1):
            ...
            logging.info(f"Gen {gen}, best_f1={best_f1:.4f}")
    ...
    for i in range(len(individual)):
        if random.random() < indpb:
            if i == 0:
                ...
                # hours_mask => فعلاً به همان شکل رها می‌کنیم
                pass
            elif i == 5:
                # lstm_units
...
if __name__=="__main__":
    ...
    # بارگذاری داده
    runner.load_and_prepare_data('XAUUSD60.csv')  # در صورت نیاز نام فایل را عوض کنید
